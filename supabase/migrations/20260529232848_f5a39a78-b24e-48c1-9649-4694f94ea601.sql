  ...
  result := jsonb_build_object(
    ...
    'top_pf', (
      ...
      from (
        ...
        order by d.pharmacy_first_count desc
        limit 10
      ) t
    ),
    'top_nms', (
    ...
    'top_eps', (
      ...
      from (
        ...
        order by d.eps_items desc
        limit 10
      ) t
    ),
    'totals_trend', (
      select coalesce(jsonb_agg(row_to_json(t) order by t.year, t.month), '[]'::jsonb)
      from (
        select
          year, month,
...
grant execute on function public.public_landing_data() to anon, authenticated, service_role;
