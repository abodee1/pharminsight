-- Add top_fastest_growing to landing cache compute function
    ...
    'top_fastest_growing', (
      select coalesce(jsonb_agg(row_to_json(t)), '[]'::jsonb) from (
        select
          ...
          p.country,
          round(
            (d_curr.items_dispensed::numeric / d_prev.items_dispensed - 1) * 100,
            1
          ) as value
        from public.dispensing_data d_curr
        join public.dispensing_data d_prev
          on d_prev.pharmacy_id = d_curr.pharmacy_id
          and d_prev.year = ly - 1
          and d_prev.month = lm
        join public.pharmacies p on p.id = d_curr.pharmacy_id
        where d_curr.year = ly
          and d_curr.month = lm
          and d_curr.items_dispensed > 0
          and d_prev.items_dispensed >= 500
        order by (d_curr.items_dispensed::numeric / d_prev.items_dispensed) desc
        limit 10
      ) t
    ),
    'totals_trend', (
...
-- Refresh cache to include new fastest-growing data
